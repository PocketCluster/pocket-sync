/*
 * Sequential Patcher will stream the patched version of the file to output, since it works strictly in order, it cannot
 * patch the local file directly (since it might overwrite a block needed later), so there would have to be a final copy
 * once the patching was done.
 */
package sequential
...
/*
 * This simple example currently doesn't do any pipelining of needed blocks, nor does it deal with blocks being
                ...
                case result := <-reference.GetResultChannel(): {
                    if result.StartBlock == currentBlock {
                        ...
                        if _, err := output.Write(result.Data); err != nil {
                            ...
                            completed := calculateNumberOfCompletedBlocks(len(result.Data), firstMissing.BlockSize)
