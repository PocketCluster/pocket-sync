/*
 * package comparer is responsible for using a FileChecksumGenerator (filechecksum) and an index to move through a file
 * and compare it to the index, producing a FileDiffSummary
 */
package comparer
...
/*
 * Iterates though comparison looking for blocks that match ones from the index. It emits each block to be read from the
 * returned channel. Callers should check for (.Err != nil) on the results, in which case reading will end immediately.

 * StartFindMatchingBlocks is capable of running asyncronously on sub-sections of a larger file. When doing this,
 * you must overlap by the block size, and use seperate checksum generators.
 */
